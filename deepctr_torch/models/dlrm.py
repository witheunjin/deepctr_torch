...
class DLRM(BaseModel):
    def __init__(self,
                 dnn_feature_columns,
                 bottom_mlp_hidden_units=(128,),
                 top_mlp_hidden_units=(1024, 512, 256),
        ...
        # Assume uniform latent_dim
        latent_dim = sparse_feature_columns[0].embedding_dim
        #print(f'latent_dim: {latent_dim}')

        # Bottom MLP
        if len(dense_feature_columns) > 0:
            self.bottom_mlp = DNN(len(dense_feature_columns), list(bottom_mlp_hidden_units) + [latent_dim], l2_reg=l2_reg_dnn, dropout_rate=dnn_dropout, use_bn=dnn_use_bn, init_std=init_std, device=device)

        # Top MLP
        self.top_mlp = DNN(len(dense_feature_columns) * latent_dim + (len(dnn_feature_columns) * (len(dnn_feature_columns) - 1)) // 2, top_mlp_hidden_units, l2_reg=l2_reg_dnn, dropout_rate=dnn_dropout, use_bn=dnn_use_bn, init_std=init_std, device=device)
    ...
    def forward(self, X):
        ...
        sparse_embedding_list, dense_value_list = self.input_from_feature_columns(X, self.dnn_feature_columns,
                                                                                  self.embedding_dict)
        #print(f'len(sparse_embedding_list): {len(sparse_embedding_list)}')
        #print(f'len(dense_value_list): {len(dense_value_list)}')
        ...
        #print(f'dnn_output.shape: {dnn_output.shape}')
    ...
    def interact_features(self, x, ly):
        ...
        #print(f'len(ly), ly[0].shape, ly[1].shape, ly[2].shape, ly[3].shape: {len(ly)}, {ly[0].shape}, {ly[1].shape}, {ly[2].shape}, {ly[3].shape}')
        ...
        _, ni, nj = Z.shape
        #print(f'_: {_}, ni: {ni}, nj: {nj}')
        li = torch.tensor([i for i in range(ni) for j in range(i)]) # ni=4, li=[1,2,2,3,3,3]
        lj = torch.tensor([j for i in range(nj) for j in range(i)])
        Zflat = Z[:, li, lj]
        #print(f'li.shape: {li.shape}')
        #print(f'lj.shape: {lj.shape}')
